...
class UpdateUserInfo:
    ...
    def getLDAPDetails(self, device):
        self.ldap_connection.search('dc=woodleighschool,dc=net', f'(samAccountName={device.jamfUser.username})', attributes=[
                                    'name', 'mail', 'samAccountName', 'userAccountControl', 'department', 'Campus'])
...
EXAMPLES:
    ...
    if not all([jamf_client_id, jamf_client_secret, jamf_host, ldap_username, ldap_credentials, ldap_host]):
        ...
        if not ldap_host:
            logging.error('  - LDAP_HOST is required')
        if not ldap_username:
            logging.error('  - LDAP_USERNAME is required')
        if not ldap_credentials:
            logging.error('  - LDAP_CREDENTIALS is required')
        logging.error('Run "updateuserinfo --help" for usage information')
        sys.exit(1)
